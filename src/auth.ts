...
import type { AuthObjectCramMd5, ConnectionContext } from "./types.ts";
...
function plainToken(
	...
	ctx.server.onAuth(
		{ method: "PLAIN", username, password },
		ctx.session,
		(err, response) => {
			if (err) {
			...
			if (!response?.user) {
				sendCode(
					ctx,
					response?.responseCode ?? 535,
					response?.message ?? "Error: Authentication credentials invalid",
				);
				return done();
			}
...
export const SASL_CRAM_MD5: AuthHandler = (ctx, args, done) => {
	...
	ctx.nextHandler = (token, next) =>
		cramMd5Token(ctx, challenge, true, token, next);
...
function xoauth2Token(
	...
	decodeB64(token)
		...
		.forEach((part) => {
			...
			if (key === "user") {
				...
				if (
					spaceIdx !== -1 &&
					value.slice(0, spaceIdx).toLowerCase() === "bearer"
				) {
	...
	ctx.server.onAuth(
		{ method: "XOAUTH2", username, accessToken },
		ctx.session,
		(err, response) => {
			if (err) {
				sendCode(ctx, err.responseCode ?? 535, err.message);
