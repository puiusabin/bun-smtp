...
const defaultOnAuth: OnAuthCallback = (_auth, _session, cb) =>
  cb(Object.assign(new Error("Error: Authentication not implemented"), { responseCode: 535 }));
...
export class SMTPServer implements ServerInstance {
  ...
  listen(...args: unknown[]): this {
    ...
    for (const arg of args) {
      if (typeof arg === "number") port = arg;
        ...
        if (opts["host"] || opts["hostname"]) listenHost = String(opts["host"] ?? opts["hostname"]);
      }
    }

    const server = this;

    const tls = (this.options.secure || this.options.needsUpgrade)
    ...
    this._listener = Bun.listen<ConnectionContext>({
      ...
      socket: {
        open(socket) {
          ...
          if (server.options.maxClients && server.connections.size > server.options.maxClients) {
            socket.write(`421 ${server.options.name} Too many connected clients, try again in a moment\r\n`);
            socket.end();
   ...
   */
  updateSecureContext(options: Pick<SMTPServerOptions, "key" | "cert" | "ca">): void {
