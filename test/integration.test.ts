...
class SMTPClient {
  ...
  async connect(port: number): Promise<string> {
    return new Promise((outerResolve) => {
      ...
      Bun.connect({
        ...
        socket: {
          open(socket) {
            client.socket = socket as unknown as Awaited<ReturnType<typeof Bun.connect>>;
          },
          ...
          close() {},
          error(_socket, err) { console.error("client error", err); },
        },
      }).then(() => {
  ...
  send(line: string): void {
    (this.socket as unknown as { write(data: string): void }).write(line + "\r\n");
  }
...
function makeServer(opts: SMTPServerOptions = {}): { server: SMTPServer; port: number } {
...
describe("onMailFrom / onRcptTo callbacks", () => {
  ...
  test("onMailFrom receives correct address and args", async () => {
    ...
    const { server, port } = makeServer({
      ...
      onMailFrom(addr, _session, cb) {
        ...
        capturedSize = (addr.args && (addr.args as Record<string, unknown>)["SIZE"] as string) ?? "";
        cb();
      },
    });
  ...
  test("session.envelope is updated after MAIL+RCPT", async () => {
    ...
    expect((envelope as { mailFrom: { address: string } }).mailFrom.address).toBe("from@test.com");
    expect((envelope as { rcptTo: Array<{ address: string }> }).rcptTo[0]?.address).toBe("to@test.com");
...
describe("authentication", () => {
  function makeAuthServer() {
    return makeServer({
      ...
      onAuth(auth, _session, cb) {
        if ("password" in auth && auth.username === "user" && auth.password === "pass") {
          cb(null, { user: { name: auth.username } });
        } else if ("validatePassword" in auth && auth.username === "user") {
          cb(null, { user: auth.validatePassword("pass") ? { name: "user" } : undefined });
        } else {
          cb(null, {});
        }
      },
  ...
  test("MAIL blocked without auth when authOptional=false", async () => {
    const { server, port } = makeServer({
      ...
      authMethods: ["PLAIN"],
      authOptional: false,
      onAuth(_auth, _session, cb) { cb(null, { user: {} }); },
    });
...
describe("onConnect hook", () => {
  ...
  test("onConnect success allows connection to proceed", async () => {
    ...
    const { server, port } = makeServer({
      disableReverseLookup: true,
      onConnect(_session, cb) { called = true; cb(); },
    });
...
describe("onClose callback", () => {
  test("onClose is called when connection closes", async () => {
    ...
    const { server, port } = makeServer({
      disableReverseLookup: true,
      onClose(session) { closedSessionId = session.id; },
    });
